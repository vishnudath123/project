...
def main_run(ImageFile):
	...
	if os.path.isdir("datasetlog") and os.path.exists("datasetlog/Datasetunlabelledlog.csv"):
		results = []
		...
		L = {'fortnum': str(fortnum), 'imgid': str(ImageFile), 'feature1': str(Tarea), 'feature2': str(Infarea), 'feature3': str(perimeter)}	
		print(L)
	...
	if(Sum < 2):
                x = "The leaf is sufficiently healthy!"
		print("The leaf is sufficiently healthy!")
	else:
                x= "The leaf is infected!"
